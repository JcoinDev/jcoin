...
#if defined(NDEBUG)
# error "Jcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x03ceef6c1438ee7bde352ce4677883fec857a131acbf693f5d20e99c821f59b5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Jcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Jcoin Signed Message:\n";
...
{
    // Jcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Jcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Jcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Jcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Jcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Jcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1e27494d6915c3c1f554acad6648a695e3115d388bde9652ee57a1b98d8cd747");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BAT gets added to coinbase";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f21ee0d4fe248e96b4310c371ae63ad3ea569b86873fb0a4374f827cde15d4b8ba9d768ad7989cfd002948defb686992e17954ec2cc3730ae7388bfcdc85e461") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1541232768;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085628874;

        if (fTestNet)
        {
            block.nTime    = 1541232743;
            block.nNonce   = 387251094;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7f952f469f46475cfbd3a77528c02dba3d065bf04a5daa522e517e1c6bace1fd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xb6, 0xdb }; // Jcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JcoinMiner(CWallet *pwallet)
{
    printf("JcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("jcoin-miner");
    ...
    try { loop {
        ...
        printf("Running JcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JcoinMiner, pwallet));
}
